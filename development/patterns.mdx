...
description: Common coding patterns and best practices in SerenityOS
---
...
`MUST(...)` is reserved for cases where we determine through other circumstances that it should not be possible for the code inside the macro to fail or if a failure is serious enough that the program _needs_ to crash.
</Warning>
...
<Note>
`Span<type>` differs from all of these types in that it provides a _view_ on data owned by somebody else. The four types mentioned below all own their data, but they can provide `Span`s which view all or part of their data.
</Note>
